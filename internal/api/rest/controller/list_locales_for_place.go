...
func (h Service) ListLocalesForPlace(w http.ResponseWriter, r *http.Request) {
	...
	locales, pagResp, err := h.domain.Place.ListLocales(r.Context(), placeID, pag)
	if err != nil {
		h.log.WithError(err).Error("failed to get place locales")
		switch {
