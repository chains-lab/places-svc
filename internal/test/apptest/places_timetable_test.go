...
func TestPlaceTimetable(t *testing.T) {
	...
	got, err := s.app.SetPlaceTimeTable(ctx, restaurant.ID, ttRestaurant)
	if err != nil {
	...
	got, err = s.app.SetPlaceTimeTable(ctx, food.ID, ttFood)
	if err != nil {
	...
	rFromDB, err := s.app.GetPlace(ctx, restaurant.ID, enum.LocaleEN)
	if err != nil {
	...
	fFromDB, err := s.app.GetPlace(ctx, food.ID, enum.LocaleEN)
	if err != nil {
	...
	if err = s.app.DeleteTimetable(ctx, restaurant.ID); err != nil {
		t.Fatalf("DeleteTimetable(restaurant): %v", err)
	}
	rAfterDel, err := s.app.GetPlace(ctx, restaurant.ID, enum.LocaleEN)
	if err != nil {
	...
	if _, err := s.app.SetPlaceTimeTable(ctx, food.ID, ttFoodReplace); err != nil {
		t.Fatalf("SetPlaceTimeTable(food replace): %v", err)
	}

	fAfterReplace, err := s.app.GetPlace(ctx, food.ID, enum.LocaleEN)
	if err != nil {
	...
	cFromDB, err := s.app.GetPlace(ctx, clothes.ID, enum.LocaleEN)
	if err != nil {
