...
func TestPlaceUpdate(t *testing.T) {
	...
	clothesNew, err := s.app.UpdatePlace(ctx, clothes.ID, clothes.Locale, app.UpdatePlaceParams{
	...
	restaurant, err = s.app.ActivatePlace(ctx, restaurant.ID, enum.LocaleUK)
	if err != nil {
	...
	restaurant, err = s.app.DeactivatePlace(ctx, restaurant.ID, enum.LocaleUK)
	if err != nil {
