...
type timetableQ interface {
	New() data.PlaceTimetablesQ
	Insert(ctx context.Context, in ...data.PlaceTimetable) error
	Upsert(ctx context.Context, in ...data.PlaceTimetable) error
	Get(ctx context.Context) (data.PlaceTimetable, error)
	Select(ctx context.Context) ([]data.PlaceTimetable, error)
	Delete(ctx context.Context) error

	FilterByID(id uuid.UUID) data.PlaceTimetablesQ
	FilterPlaceID(placeID uuid.UUID) data.PlaceTimetablesQ
	FilterBetween(startMin, endMin int) data.PlaceTimetablesQ

	Count(ctx context.Context) (uint64, error)
	Page(limit, offset uint64) data.PlaceTimetablesQ
}

func (m Module) SetTimetable(
	ctx context.Context,
	placeID uuid.UUID,
	...
	stmt := make([]data.PlaceTimetable, 0, len(intervals.Table))
	for _, interval := range intervals.Table {
		start, end := interval.ToNumberMinutes()
		stmt = append(stmt, data.PlaceTimetable{
			ID:       uuid.New(),
			PlaceID:  placeID,
			StartMin: start,
			EndMin:   end,
...
func (m Module) GetTimetable(ctx context.Context, placeID uuid.UUID) (models.Timetable, error) {
...
func (m Module) DeleteTimetable(ctx context.Context, placeID uuid.UUID) error {
